...
- 본 대회에서는 학습 및 평가 데이터셋이 제공되지 않습니다
- 따라서 참가자가 직접 데이터셋을 구성해야 합니다
- 평가 데이터셋의 경우, 상용 동영상 / 이미지 생성 서비스와 오픈소스 모델 대부분이 사용되었으며, 구형 face swap, lip sync 방식도 포함되어 있습니다
- 본 대회는 추론 결과가 아니라, 모델을 제출하여 자동 추론 방식으로 평가를 수행합니다

## 🔵 수행 절차 및 방법

### 수행 절차

1. 제공된 baseline을 기반으로 task부터 제출까지의 전체적인 흐름 이해
2. 어떤 데이터셋을 활용할지 탐색
3. 어떤 모델을 사용할지 탐색
4. 구성한 데이터셋을 기반으로 선택한 모델 학습 및 제출 진행
5. ...
...
### baseline을 통한 전반적인 흐름 이해

가장 먼저 대회에서 제공되는 baseline을 통해 학습부터 추론 및 제출까지 이어지는 흐름을 파악하였습니다
...
아쉬운 점으로는, 데이터셋의 크기(72.8 GB)가 조금 커서 전체를 사용하지 못하고 `test` 데이터셋만 사용하여 학습을 진행하였습니다. 이로 인해 원래 데이터셋의 정보가 온전히 전달되지 않은 영향인지 baseline에 비해 유의미한 성능 향상을 볼 수 없었습니다.
...
<img src="https://i.ibb.co/Mxvmp69g/image.png" alt="image" border="0">
...
`ViT` 모델에 대해 `transformers`라이브러리를 활용하여 Fine Tuning을 진행했습니다. 한정된 자원으로 인해, Classifier를 제외하고 가중치를 업데이트할 layer도 많지 않았습니다. 그럼에도 baseline보다는 약간 성능이 향상된 모습을 보였습니다.

앞서 말한 한정된 자원으로 인해 `ViT`보다 조금 더 가벼운 모델을 사용해보기 위해 `ConvNeXt V2` 모델을 사용해보았습니다. `V2` 아키텍처가 직접적으로 딥페이크 탐지 task에 사용된 사례는 아직 드물지만 `ConvNeXt 계열(CNN)`은 딥페이크 탐지에 실제로 사용된 사례가 존재했습니다. 이를 근거로, `ConvNeXt-V2`가 딥페이크 탐지 task에서 유리한 지점이 있지 않을까 판단했습니다. 결과적으로는 baseline보다 떨어진 성능을 보였습니다. 이는 가중치를 업데이트할 layer를 설정하고 Fine Tuning 과정에서 표현력 제한이나 저수준 특징 학습 부족, 사전 학습 데이터 분포와의 차이 등이 원인이 될 수 있겠습니다.
